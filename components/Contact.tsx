import { ArrowUpRight, CircleCheck, Clock, ImageUp, MessageCircle, PhoneCall } from "lucide-react";
import { contact, contactSection } from "@/lib/content";
import { FadeUp } from "./FadeUp";
import { sectionTitle } from "./ui";
import { SectionLabel } from "./SectionLabel";

const icons = { tel: PhoneCall, kakao: MessageCircle, sms: ImageUp };
                  ...
                  return (
                    <li key={action.key}>
                      <a
                        ...
                        className={`group flex min-h-[84px] items-center gap-4 rounded-2xl px-5 transition duration-300 ease-out-soft hover:-translate-y-0.5 md:px-6 ${
                          primary
                      ...
                      >
                        <span
                          className={`grid size-12 shrink-0 place-items-center rounded-full ${primary ? "bg-white/20" : "bg-white/8"}`}
                          aria-hidden
                        >
                          <Icon className="size-[22px]" strokeWidth={1.8} />
                        </span>
                        <span className="flex-1">
                          <span className="block text-lg font-bold tracking-[-0.02em]">{action.label}</span>
                          <span className={`block text-sm ${primary ? "text-white/85 tabular-nums" : "text-white/55"}`}>
                            {action.sub}
                          </span>
                        </span>
                        <ArrowUpRight
